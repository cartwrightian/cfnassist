...
public class CloudClient { //implements ProgressListener {
    ...
    public Vpc describeVpc(final String vpcId) {
    ...
    public void addTagsToResources(final List<String> resources, final List<Tag> tags) {
        CreateTagsRequest createTagsRequest = CreateTagsRequest.builder().resources(resources).tags(tags).build();
        ec2Client.createTags(createTagsRequest);
    }

    public void deleteTagsFromResources(final List<String> resources, final Tag tag) {
    ...
    public Map<String, AvailabilityZone> getAvailabilityZones() {
        ...
        final Filter filter = Filter.builder().name("region-name").values(region.id()).build();
        DescribeAvailabilityZonesRequest request = DescribeAvailabilityZonesRequest.builder().
                filters(filter).build();


        final DescribeAvailabilityZonesResponse result = ec2Client.describeAvailabilityZones(request);
        final List<AvailabilityZone> zones = result.availabilityZones();
        logger.info(format("Found %s zones", zones.size()));

        final Map<String, AvailabilityZone> zoneMap = new HashMap<>();
        zones.forEach(zone -> zoneMap.put(zone.zoneName().replace(zone.regionName(), ""), zone));
        return zoneMap;
    }

    public software.amazon.awssdk.services.ec2.model.Instance getInstanceById(final String id) throws WrongNumberOfInstancesException {
        final DescribeInstancesRequest request = DescribeInstancesRequest.builder().instanceIds(id).build();
        final DescribeInstancesResponse result = ec2Client.describeInstances(request);
        final List<Reservation> res = result.reservations();
        if (res.size() != 1) {
            throw new WrongNumberOfInstancesException(id, res.size());
        }
        List<software.amazon.awssdk.services.ec2.model.Instance> ins = res.getFirst().instances();
        if (ins.size() != 1) {
    ...
    public void addIpsToSecGroup(final String secGroupId, final Integer port, final List<InetAddress> addresses) {
    ...
    public void deleteIpFromSecGroup(final String groupId, final Integer port, final List<InetAddress> addresses) {
    ...
    private Collection<IpPermission> createPermissions(final Integer port, final List<InetAddress> addresses) {
...
//    }
    ...
    public AWSPrivateKey createKeyPair(final String keypairName) {
